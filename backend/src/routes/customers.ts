...
export const mapCustomer = (c: any) => ({
  id: c.id,
  name: c.name,
...
router.get('/', async (req: Request, res: Response) => {
  try {
    ...
    return res.status(200).json(customers.map(mapCustomer));
  } catch (error: any) {
...
router.get('/:id', async (req: Request, res: Response) => {
  try {
    ...
    const simulations = await db.all(
      'SELECT * FROM simulations WHERE customer_id = ? ORDER BY created_at DESC',
      customerId
    );
...
router.post('/', async (req: Request, res: Response) => {
  try {
    ...
    try {
      ...
      return res.status(201).json(mapCustomer(newCustomer));
    } catch (dbError: any) {
